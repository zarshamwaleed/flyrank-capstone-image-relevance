        ...
        try:
            ...
            for image in images:
                try:
                    ...
                    if embedding:
                        results["success"] += 1
                        results["details"].append({"image_id": image.id, "filename": image.filename, "status": "success"})
                    else:
                        results["failed"] += 1
                        results["details"].append({"image_id": image.id, "filename": image.filename, "status": "failed"})
                except Exception as e:
                    results["failed"] += 1
                    results["details"].append({"image_id": image.id, "filename": image.filename, "status": "failed", "error": str(e)})
        ...
        try:
            ...
            for post in posts:
                try:
                    ...
                    if updated:
                        results["success"] += 1
                        results["details"].append({"post_id": post.id, "title": post.title, "status": "success"})
                    else:
                        results["failed"] += 1
                        results["details"].append({"post_id": post.id, "title": post.title, "status": "failed"})
                except Exception as e:
                    results["failed"] += 1
                    results["details"].append({"post_id": post.id, "title": post.title, "status": "failed", "error": str(e)})
